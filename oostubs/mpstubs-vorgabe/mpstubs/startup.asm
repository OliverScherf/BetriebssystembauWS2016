; $Id: startup.asm,v 1.3 2004/02/12 17:52:53 lohmann Exp $
...
[GLOBAL system_gdt_descr]
[GLOBAL system_gdt_start]
[GLOBAL system_gdt_end]
[GLOBAL system_idt_descr]
[GLOBAL system_idt_start]
[GLOBAL system_idt_end]
[GLOBAL system_code_start]
[GLOBAL bootinfo_pointer]
...
;
	lgdt	[system_gdt_descr]
	mov     dword [bootinfo_pointer], ebx
	jmp	dword (selector_pmcode-system_gdt_start):floppy_boot

floppy_boot:
; Globales Datensegment
	mov	ax,selector_pmdata - system_gdt_start
	mov	ds,ax
	mov	es,ax
	mov	fs,ax
	mov	gs,ax
...
;
	lgdt	[system_gdt_descr]
	lidt	[system_idt_descr]
	
; Aufruf des C-Codes

	call	main_ap		; C/C++ Level System
	hlt
...
setup_idt:
	...
	mov	ecx,256		; Zaehler
.loop:	add	[system_idt_start+8*ecx+0],ax
	adc	[system_idt_start+8*ecx+6],bx
	dec	ecx
	jge	.loop
...
system_gdt_descr:
	dw	system_gdt_end - system_gdt_start ; GDT Limit
	dd	system_gdt_start	; Physikalische Adresse der GDT
;
;  'interrupt descriptor table' mit 256 Eintraegen.
;
system_idt_start:

%macro system_idt_entry 1
	dw	(wrapper_%1 - wrapper_0) & 0xffff ; Offset 15..0
	dw	(selector_pmcode - system_gdt_start)    ; Segment selector
	dw	0xee00                            ; Present, DPL 0, 32-bit interrupt gate
...
system_code_start:
	dw selector_pmdata - system_gdt_start

;   Stack im BSS Bereich
[SECTION .bss]
